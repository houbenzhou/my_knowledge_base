...
def tianzhibei2_to_voc(source_images, source_label, out_data):
    """
    ...
    sda_path = os.path.join(out_data, "VOC.sda")
    ...
    for source_images_filename in source_images_filenames:
        ...
        source_label_filename=temp_source_images_filename_tuple[0]+".json"
        ...
        # json to xml
        json_file = json.load(open(source_label_file, "r", encoding="utf-8"))
        height, width, channels = json_file['imageHeight'], json_file['imageWidth'], 3
        tile_size=height
        tile_size_offset=height/2
    ...
    # 保存文件索引
    _save_index_file(out_main_path, source_label)
...
def _save_sda_file(source_images_path, source_label_path, sda_file,tile_shape=None):
    source_images_filenames = os.listdir(source_images_path)
    class_names=[]

    tile_size, tile_size = tile_shape
    tile_size_offset = tile_size-100
    for source_images_filename in source_images_filenames:
    ...
    save_config_to_yaml(dic_voc_yml, sda_file)

if __name__ == '__main__':
    voc_path=r'E:\workspaces\iobjectspy_master\resources_ml\out\tainzhibei2\tzb2_airplane_voc_modify_v2_20210806\voc_600'
    source_images = os.path.join(voc_path,'Images')
    source_label = os.path.join(voc_path,'Annotations')
    out_main_path = os.path.join(voc_path, "ImageSets", "Main")
    sda_file=os.path.join(voc_path,os.path.basename(voc_path)+'.sda')
    if not os.path.exists(out_main_path):
        os.makedirs(out_main_path)

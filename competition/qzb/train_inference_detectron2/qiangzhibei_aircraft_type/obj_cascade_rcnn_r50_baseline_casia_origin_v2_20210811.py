...
def example_train():
    train_data_path = '/home/data1/competition/data/all_aircraft/CASIA_Aircraft_voc_v2_0_20210812/origin_voc'
    config = os.path.join(curr_dir, '..','config','qiangzhibei_aircraft_type', 'train_config_cascade_rcnn_R_50_FPN_casia.sdt')
    backbone_weight_path = os.path.join(curr_dir, '..', 'backbone', 'R-50.pkl')

    log_path = os.path.join(curr_dir, '..', 'out', 'CASIA_Aircraft_voc_v2_0_20210812', 'baseline','log')
    output_model_path = os.path.join(curr_dir, '..', 'out', 'CASIA_Aircraft_voc_v2_0_20210812', 'baseline', 'model')
    output_model_name = "save_model"
    lr= 0.02
    PictureTrainer(train_data_path=train_data_path, config=config, epoch=50, batch_size=32, lr=lr,
...
def example_predict():
    ...
    model_path = os.path.join(curr_dir, '..', 'out', 'CASIA_Aircraft_voc_v2_0_20210812', 'baseline', 'model_600', 'save_model',
                              'save_model.sdm')
    out_data = os.path.join(image_path,'casia_result_600.udbx')

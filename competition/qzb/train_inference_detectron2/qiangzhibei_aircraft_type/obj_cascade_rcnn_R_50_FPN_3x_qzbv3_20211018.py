...
def example_train():
    train_data_path = '/home/data1/competition/data/all_aircraft/qzb/qzbv3'
    config = os.path.join(curr_dir, '..','config','qiangzhibei_aircraft_type', 'train_config_cascade_rcnn_R_50_FPN_3x_qzbv3.sdt')
    backbone_weight_path = os.path.join(curr_dir, '..', 'backbone', 'R-50.pkl')

    log_path = os.path.join(curr_dir, '..', 'out', 'qzbv3', 'cascade_rcnn_R_50_FPN_3x','log')
    output_model_path = os.path.join(curr_dir, '..', 'out', 'qzbv3', 'cascade_rcnn_R_50_FPN_3x', 'model')
    output_model_name = "save_model"
    lr= 0.0025
    PictureTrainer(train_data_path=train_data_path, config=config, epoch=100, batch_size=2, lr=lr,
            output_model_path=output_model_path,
            output_model_name=output_model_name, backbone_name='renet50',
            backbone_weight_path=backbone_weight_path, log_path=log_path, reload_model=True,num_gpus=1).object_detect_train()

def example_predict():
    image_path = '/home/data1/competition/data/all_aircraft/qinagzhibei_test'
    image_names = os.listdir(image_path)
    model_path = os.path.join(curr_dir, '..', 'out', 'tzb2_airplane_voc_modify_v2_20210806', 'baseline', 'model', 'save_model',
                              'save_model.sdm')

    out_data = os.path.join(curr_dir, '..', 'out', 'tzb2_airplane_voc_modify_v2_20210806', 'baseline', 'result.udbx')

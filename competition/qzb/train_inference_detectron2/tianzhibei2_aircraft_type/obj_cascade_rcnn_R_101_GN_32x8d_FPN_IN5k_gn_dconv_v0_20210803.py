...
def example_train():
    train_data_path = '/home/data1/competition/data/all_aircraft/tzb2_airplane_voc_v0_20210803/splite_voc_600_500'

    config = os.path.join(curr_dir, '..','config','tianzhibei2_aircraft_type', 'train_config_cascade_rcnn_R_101_GN_32x8d_FPN_IN5k_gn_dconv.sdt')
    backbone_weight_path = os.path.join(curr_dir, '..', 'backbone', 'R-101-GN.pkl')

    log_path = os.path.join(curr_dir, '..', 'out','log', 'tianzhibei2_aircraft_type','cascade_rcnn_R_101_GN_32x8d_FPN_IN5k_gn_dconv', 'splite_voc_600')
    output_model_path = os.path.join(curr_dir, '..', 'out','model', 'tianzhibei2_aircraft_type','cascade_rcnn_R_101_GN_32x8d_FPN_IN5k_gn_dconv', 'splite_voc_600')
    output_model_name = "save_model"

    lr= 0.01
    PictureTrainer(train_data_path=train_data_path, config=config, epoch=50, batch_size=2, lr=lr,
            output_model_path=output_model_path,
            output_model_name=output_model_name, backbone_name='renet50',
            backbone_weight_path=backbone_weight_path, log_path=log_path, reload_model=False,num_gpus=1).object_detect_train()
...
def example_predict():
    ...
    model_path = os.path.join(curr_dir, '..', 'out', 'qiangzhibei_aircraft_type','cascade_rcnn_R_101_GN_32x8d_FPN_IN5k_gn_dconv', 'splite_voc_600', 'model')

    out_data = os.path.join(image_path,'result_cascade_rcnn_R_101_GN_32x8d_FPN_IN5k_gn_dconv.udbx')

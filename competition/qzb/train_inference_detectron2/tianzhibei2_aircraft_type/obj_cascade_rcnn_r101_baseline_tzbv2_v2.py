...
def example_train():
    ...
    config = os.path.join(curr_dir, '..','config','tianzhibei2_aircraft_type', 'train_config_cascade_rcnn_R_101_FPN_3x_tzbv2_v2.sdt')
    backbone_weight_path = os.path.join(curr_dir, '..', 'backbone', 'R-101.pkl')

    log_path = os.path.join(curr_dir, '..', 'out', 'tzbv2', 'baseline_r101_v2','log')
    output_model_path = os.path.join(curr_dir, '..', 'out', 'tzbv2', 'baseline_r101_v2', 'model')
    output_model_name = "save_model"
    lr= 0.01
    pretrained_model_path='/home/data1/hou/workspaces/iobjectspy_master/resources_ml/competition/train_inference_detectron2/out/tzbv2/baseline_r101/model/save_model/save_model.pth'
    ...
    PictureTrainer(train_data_path=train_data_path, config=config, epoch=50, batch_size=8, lr=lr,
            ...
            backbone_weight_path=backbone_weight_path, pretrained_model_path=pretrained_model_path,log_path=log_path, reload_model=True,num_gpus=2).object_detect_train()

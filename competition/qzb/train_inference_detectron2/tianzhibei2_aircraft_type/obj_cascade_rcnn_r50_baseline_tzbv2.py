...
def example_train():
    train_data_path = r'/home/data1/competition/data/all_aircraft/tzb2_airplane_voc_v0_20210803/tzbv2'
    config = os.path.join(curr_dir, '..','config','tianzhibei2_aircraft_type', 'train_config_cascade_rcnn_R_50_FPN_3x_tzbv2.sdt')
    backbone_weight_path = os.path.join(curr_dir, '..', 'backbone', 'R-50.pkl')

    log_path = os.path.join(curr_dir, '..', 'out', 'tzbv2', 'baseline','log')
    output_model_path = os.path.join(curr_dir, '..', 'out', 'tzbv2', 'baseline', 'model')
    output_model_name = "save_model"
    lr= 0.01
    pretrained_model_path='/home/data1/hou/workspaces/iobjectspy_master/resources_ml/competition/train_inference_detectron2/out/tzbv2/baseline/log/2021-10-14/object_detection_cascade_rcnn/model_0282034_21-18-56.pth'
    PictureTrainer(train_data_path=train_data_path, config=config, epoch=50, batch_size=8, lr=lr,
            output_model_path=output_model_path,
            output_model_name=output_model_name, backbone_name='renet50',
            backbone_weight_path=backbone_weight_path,pretrained_model_path=pretrained_model_path, log_path=log_path, reload_model=False,num_gpus=2).object_detect_train()

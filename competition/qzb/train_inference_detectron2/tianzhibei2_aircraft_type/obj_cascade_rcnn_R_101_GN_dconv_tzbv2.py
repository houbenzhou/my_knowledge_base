...
def example_train():
    train_data_path = '/home/data1/competition/data/all_aircraft/tzb2_airplane_voc_v0_20210803/tzbv2'

    config = os.path.join(curr_dir, '..','config','tianzhibei2_aircraft_type', 'train_config_cascade_rcnn_R_101_GN_dconv_tzbv2.sdt')
    backbone_weight_path = os.path.join(curr_dir, '..', 'backbone', 'R-101-GN.pkl')

    log_path = os.path.join(curr_dir, '..', 'out', 'tzbv2', 'cascade_rcnn_R_101_GN_dconv', 'log')
    output_model_path = os.path.join(curr_dir, '..', 'out', 'tzbv2', 'cascade_rcnn_R_101_GN_dconv', 'model')
    output_model_name = "save_model"

    lr= 0.005
    PictureTrainer(train_data_path=train_data_path, config=config, epoch=50, batch_size=4, lr=lr,
            output_model_path=output_model_path,
            output_model_name=output_model_name, backbone_name='renet101',
            backbone_weight_path=backbone_weight_path, log_path=log_path, reload_model=True,num_gpus=2).object_detect_train()

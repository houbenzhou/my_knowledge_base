...
if __name__ == "__main__":
    ...
    wg_match_dir = r"E:\new_iob\image-matching\out_dir\wrong" # 匹配错误文件夹
    ...
    out_txt_dir1 = os.path.join(out_match_dir, 'txt1')
    out_plot_dir1 = os.path.join(out_match_dir, 'match_plot1')
    os.makedirs(out_txt_dir1, exist_ok=True)
    os.makedirs(out_plot_dir1, exist_ok=True)
    ...
    ct_imgs = os.listdir(out_match_dir)
    ct_imgs = list(filter(lambda x: x.endswith('png'), ct_imgs)) # 匹配正确列表
    wg_imgs = os.listdir(wg_match_dir)
    wg_imgs = list(filter(lambda x: x.endswith('png'), wg_imgs)) # 匹配错误列表


    for ct_img in ct_imgs: # 从 list中删除匹配正确的影像
        ct_img = ct_img.split('.')[0].split('-')
        print(ct_img)
        for tes in test_imgs:
            t = tes.split('-')
            if t[0] == ct_img[1]:
                test_imgs.remove(tes)
        for bes in test_imgs:
            b = bes.split('-')
            if b[0] == ct_img[0]:
                base_imgs.remove(bes)
    for wg_img in wg_imgs: # 删除匹配错误的计算结果
        os.remove(out_txt_dir + '/' + wg_img[:-4] + '.txt')
        os.remove(out_plot_dir + '/' + wg_img[:-4] + '.png')
        os.remove(out_lefttop_coordinate_dir + '/' + wg_img[:-4] + '.txt')
        os.remove(out_transform_dir + '/' + wg_img[:-4] + '.txt')
    

    print('begin to match...')
    for imgs in test_imgs:
        multiprocess_match(test_resize_img_dir, base_resize_img_dir, base_imgs, out_txt_dir1, out_plot_dir1, imgs)

    save_picture_dir = out_match_dir #存储精匹配结果路径

    rela_imgs = os.listdir(out_plot_dir1)
    rela_imgs = list(filter(lambda x: x.endswith('png'), rela_imgs))

    for _rela in rela_imgs: # 开始精匹配
        trans_coordinate(_rela, out_txt_dir1, input_base_image, input_test_image, save_picture_dir, out_transform_dir,

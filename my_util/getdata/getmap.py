...
if __name__ == '__main__':
    ...
    for row in csv_reader:
        ...
        # samLR = mecator_to_wgs(samSiteX + 300, samSiteY + 300)
        #left
        samUL = mecator_to_wgs(samSiteX - 900, samSiteY - 300)
        samLR = mecator_to_wgs(samSiteX - 300, samSiteY + 300)
        #right
        # samUL = mecator_to_wgs(samSiteX + 900, samSiteY - 300)
        # samLR = mecator_to_wgs(samSiteX + 1500, samSiteY + 300)
        #below
        # samUL = mecator_to_wgs(samSiteX - 300, samSiteY - 600)
        # samLR = mecator_to_wgs(samSiteX + 300, samSiteY - 1200)
        try:
            # SAM_SITE数据获取
            # x = getpic(double(samLR[0]), double(samLR[1]), double(samUL[0]), double(samUL[1]), 20, source='google',style='m', outfile="/home/hou/python/" + str(n) + ".png")
            x = getpic(double(samLR[0]), double(samLR[1]), double(samUL[0]), double(samUL[1]), 20, source='google',
        style = 'm', outfile = "E;//" + str(n) + ".png")
        except OSError:
            print()
    ...
    #     samSiteY=double(row[10])
    #     n=n+1
    #     x = getpic(samSiteX-300,samSiteY+300, samSiteX+300,samSiteY-300,
    #             17, source='google', style='m', outfile=str(n)+".jpg")
